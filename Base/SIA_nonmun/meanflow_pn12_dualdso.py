...
class MeanFlowTSE:
    ...
    def loss_mr_jitter(
        ...
        #
        # Original MR-jitter optimizes velocity-space error:
        #
        #     error_u = pred - target
        #
        # where:
        #
        #     target = (source - mixture) / (1 - m_hat)
        #
        # PN11 instead optimizes the actual endpoint displacement error:
        #
        #     error_x = (1 - m_hat) * error_u
        #
        #             = mixture
        #               + (1 - m_hat) * pred
        #               - source
        #
        # Thus time sampling, MR-jitter, model conditioning and target
        # velocity are IDENTICAL to V13R. Only the optimization space
        # changes from velocity-space to data/endpoint-space.
        # ==============================================================

        dso_error = denom.to(dtype=error.dtype) * error

        # --------------------------------------------------------------
        # PN12 part 1: MR-Jitter endpoint DSO (same as PN11)
        # --------------------------------------------------------------
        ...
        # Keep original raw velocity MSE for direct V13R comparison.
        mse_val = (error ** 2).mean()
