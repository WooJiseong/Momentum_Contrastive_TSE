...
class NoisyFlowTSEDataset(Dataset):
    ...
    def __init__(
        ...
        # ------------------------------------------------------------
        # PN7 Hard-negative mixture sampling.
        #
        # IMPORTANT:
        #   - training only (reproducable=False)
        #   - validation/test stay byte-identical to the original sampler
        # ------------------------------------------------------------
        self.hard_negative_prob = float(hard_negative_prob)
        self.hard_negative_pool_size = max(1, int(hard_negative_pool_size))
        self.hard_negative_map_path = str(hard_negative_map_path or "")
        self.hard_negative_map = {}

        if not 0.0 <= self.hard_negative_prob <= 1.0:
            raise ValueError(
                f"hard_negative_prob must be in [0,1], got {self.hard_negative_prob}"
            )

        if (not self.reproducable) and self.hard_negative_prob > 0.0:
            if not self.hard_negative_map_path:
                raise ValueError(
                    "PN7 hard_negative_prob > 0 but no hard_negative_map_path was provided"
                )

            if not os.path.isfile(self.hard_negative_map_path):
                raise FileNotFoundError(
                    f"PN7 hard-negative map not found: {self.hard_negative_map_path}"
                )

            with open(self.hard_negative_map_path, "r") as f:
                raw_map = json.load(f)

            for target_pid, rows in raw_map.items():
                candidates = []
                for row in rows:
                ...
                self.hard_negative_map[str(target_pid)] = candidates

            print(
                "[PN7] hard-negative sampler enabled: "
                f"prob={self.hard_negative_prob:.3f}, "
                f"pool={self.hard_negative_pool_size}, "
                f"targets={len(self.hard_negative_map)}, "
                f"map={self.hard_negative_map_path}",
                flush=True,
            )
    ...
    def _sample_mixture_interferers(
        self,
        tgt_pid: str,
        other_person_ids,
        k: int,
    ):
        """PN7: mixture interferers only.

        With probability hard_negative_prob, sample all mixture interferers
        from the target speaker's nearest-speaker pool.

        Validation/test are always original random sampling because
        reproducable=True disables this path.
        """
        k = int(k)
        ...
        if (
            self.reproducable
            or self.hard_negative_prob <= 0.0
            or not self.hard_negative_map
            or random.random() >= self.hard_negative_prob
        ):
            return random.sample(other_person_ids, k)

        valid = set(other_person_ids)

        ranked = [
            pid
            for pid in self.hard_negative_map.get(str(tgt_pid), [])
            if pid in valid and pid != str(tgt_pid)
        ]

        hard_pool = ranked[: self.hard_negative_pool_size]

        # If this target is absent from the map, preserve baseline behavior.
        if not hard_pool:
            return random.sample(other_person_ids, k)

        n_hard = min(k, len(hard_pool))
        picked = random.sample(hard_pool, n_hard)

        if len(picked) < k:
            remaining = [
                pid
                for pid in other_person_ids
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        sample_noise_pids = self._sample_mixture_interferers(
            tgt_pid,
            other_person_ids,
            source_num - 1,
        )
