...
from pndata.online_mixer_pn21_curriculum import NoisyFlowTSEDataset
...
def _build_inner(root_dir: str, noise_dir: str, dcfg: dict, reproducable: bool) -> NoisyFlowTSEDataset:
    ...
    return NoisyFlowTSEDataset(
        ...
        # PN7 only. Explicitly OFF for reproducable val/test.
        hard_negative_map_path=(
            _abs(str(dcfg.get("hard_negative_map_path")))
            if dcfg.get("hard_negative_map_path")
            else ""
        ),
        hard_negative_prob=(
            0.0
            if reproducable
            else float(dcfg.get("hard_negative_prob", 0.0))
        ),
        hard_negative_pool_size=int(
            dcfg.get("hard_negative_pool_size", 20)
        ),
    )
