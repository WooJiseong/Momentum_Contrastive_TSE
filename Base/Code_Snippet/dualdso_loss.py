...
class MeanFlowTSE:
    ...
    def loss_mr_jitter(
        ...
        # ==============================================================
        # PN12 part 2: trajectory DSO
        #
        # Keep the SAME source/background path as V13R, but additionally
        # train random intermediate t -> r displacements.
        #
        # mixture = (1-m) * background + m * source
        # therefore:
        # background = (mixture - m*source) / (1-m)
        # ==============================================================

        if self.data_dim == '1d':
            m_view = rearrange(m, "b -> b 1 1")
        else:
            m_view = rearrange(m, "b -> b 1 1 1")
        ...
        reconstructed_background = (
            mixture - m_view * source
        ) / one_minus_m

        # Same logistic t/r sampling family used in MeanFlow alpha-flow.
        mu, sigma_traj = -0.4, 1.0

        # Keep trajectory sampling on the active device.  The previous
        # NumPy->Torch path ran inside Lightning's forked DDP workers and
        # could terminate one rank at epoch boundaries without a traceback.
        normal_samples = (
            torch.randn(
                batch_size,
                2,
        ...
        r_traj = torch.maximum(samples[:, 0], samples[:, 1])
        ...
        x_t = (
            (1.0 - t_view) * reconstructed_background
            + t_view * source
        )

        trajectory_target = (
            source - reconstructed_background
        )

        # Evaluate the endpoint and trajectory branches as one concatenated
        # batch. This is algebraically identical to two independent calls,
        # but presents one deterministic forward graph to DDP per step.
        joint_pred = model(
            torch.cat([mixture, x_t], dim=0),
            torch.cat([m_hat, t], dim=0),
            torch.cat([r, r_traj], dim=0),
            enrollment=torch.cat([enrollment, enrollment], dim=0),
        )
        pred, trajectory_pred = joint_pred.split(batch_size, dim=0)
        error = pred - target

        # ==============================================================
        # PN11 — Data-Space Optimization (DSO)
        # ==============================================================
        dso_error = denom.to(dtype=error.dtype) * error
        mr_dso_loss = adaptive_l2_loss(
            dso_error,
            gamma=self.gamma,
            alpha=1.0,
        )
        mse_val = (error ** 2).mean()
        dso_mse_val = (dso_error ** 2).mean()
        ...
        trajectory_dso_error = (
            delta_view.to(
                dtype=trajectory_error.dtype
            )
            * trajectory_error
        )

        # PN8-style plain displacement MSE.
        trajectory_dso_loss = (
            trajectory_dso_error
            .square()
            .mean()
        ...
        # Equal weighting for first full experiment.
        trajectory_dso_weight = 1.0

        loss = (
            mr_dso_loss
            + trajectory_dso_weight
            * trajectory_dso_loss
        )

        stats = {
            # PN11/PN12 endpoint DSO diagnostics
            'dso_displacement_mse': dso_mse_val.detach(),
            'dso_delta_mean': (1.0 - m_hat).detach().mean(),
            'dso_delta2_mean': ((1.0 - m_hat) ** 2).detach().mean(),

            # PN12 trajectory DSO diagnostics
            'trajectory_dso_loss': trajectory_dso_loss.detach(),
            'trajectory_dso_weight': trajectory_dso_loss.detach().new_tensor(
                trajectory_dso_weight
            ),
            'trajectory_delta_mean': trajectory_delta.detach().mean(),
            'trajectory_delta2_mean': (
                trajectory_delta.detach().square().mean()
            ),
