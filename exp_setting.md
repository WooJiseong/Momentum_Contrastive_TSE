...
`/gpfs/home1/wjs6800/SIALab/PNFlowTSE/sia_fm_tse`는 외부 참조 레포지토리이며 읽기 전용으로 취급한다.

허용되는 작업:

* 원본 decoder, MeanFlow, inference 및 evaluation 구현 확인
* 함수와 클래스의 동작을 실험 Lab에서 import하여 사용
* 설정 형식과 평가 절차 참조

금지되는 작업:

* `sia_fm_tse` 내부 파일 수정, 삭제, 이동 또는 자동 포맷팅
* 외부 레포지토리 내부에 로그, 체크포인트, 평가 결과 또는 임시 파일 생성
* 외부 레포지토리의 코드를 실험 조건 기록 없이 직접 변경

`sia_fm_tse`의 동작을 실험에 맞게 조정해야 하는 경우에는 다음 원칙을 따른다.

1. `Running_Lab/<Lab_Name>/` 안에 wrapper, adapter, config 또는 실행 script를 작성한다.
2. 외부 레포지토리의 원본 파일은 수정하지 않고, 필요한 차이점은 Lab의 `README.md` 또는 `setting_history.md`에 기록한다.
3. 로그, 체크포인트, 평가 결과 및 TensorBoard 파일은 반드시 `contrastive_momentum` 내부 해당 실험의 `exp/YYYYMMDD_실험제목/` 아래에 저장한다.
4. 외부 레포지토리 코드와 실험 Lab 코드의 동작이 다르면, 참조한 파일과 적용한 wrapper/adapter를 명시한다.

현재 MeanFlow 실험에서는 `Running_Lab/MeanFlow_MOCOCO/`가 `sia_fm_tse`의 구현을 참조하며, 외부 레포지토리 자체는 수정하지 않는다.
