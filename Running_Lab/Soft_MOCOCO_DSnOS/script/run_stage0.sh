...
export NUMBA_CACHE_DIR="${NUMBA_CACHE_DIR:-/tmp/soft_mococo_dsnos_numba_${USER:-user}}"
...
if [[ -n "${RESUME_CKPT:-}" ]]; then
    ...
    echo "[run_stage0] Resuming from: $RESUME_CKPT"
fi

exec "$PYTHON_BIN" "$LAB_DIR/train_soft_moco_encoder_dsnos.py" "${ARGS[@]}"
