...
base_spec = importlib.util.spec_from_file_location(
    "pn_mococo_base_train_tfgridnet", pn_mococo / "train_tfgridnet.py"
)
...
from pn_soft_mococo.losses import target_orthogonal_leakage_loss
from pn_soft_mococo.stage1_data import get_soft_leakage_dataloaders


class SoftStage1LightningModule(base_train.LightningModule):
    """Stage1 separator training with Teacher-compatible anti-leakage regularization."""
    ...
    def _nuisance_sources(
        ...
        if "nuisance_sources" not in batch:
            raise KeyError(
                "Batch is missing 'nuisance_sources'. Leakage loss must receive the individual "
                "waveforms from sample[1:], never negative enrollment or aggregate background."
            )
        nuisance_sources = batch["nuisance_sources"]
        if nuisance_sources.ndim != 3:
            raise ValueError(
                f"nuisance_sources must be [B, J, T], got {tuple(nuisance_sources.shape)}"
            )
        if nuisance_sources.shape[0] != target.shape[0] or nuisance_sources.shape[-1] != target.shape[-1]:
            raise ValueError(
                "nuisance_sources must have the same batch and time dimensions as source: "
        ...
        if self.verify_nuisance_contract and not self._nuisance_contract_checked:
            ...
            max_reference = expected_background.abs().amax().clamp_min(1.0)
            allowed_error = 1e-5 * max_reference
            if not torch.isfinite(nuisance_sources).all() or max_error > allowed_error:
                raise RuntimeError(
                    "Invalid leakage-loss nuisance contract: sum(nuisance_sources) must reconstruct "
                    f"mixture - source. max_error={float(max_error):.3e}, "
                    f"allowed={float(allowed_error):.3e}"
                )
            self._nuisance_contract_checked = True
        return nuisance_sources, reconstruction_mae

    def _soft_step(self, batch: dict, train: bool):
        ...
        if self.fail_on_nonfinite and not torch.isfinite(total).all():
            raise FloatingPointError(
                "Non-finite Stage1 loss: "
                f"si_sdr={float(base_loss.detach()):.6g}, "
                f"leakage={float(leakage_loss.detach()):.6g}, "
                f"weight={self.orthogonal_weight}"
            )
    ...
    def training_step(self, batch, batch_idx):
        return self._soft_step(batch, train=True)

    def validation_step(self, batch, batch_idx):
        return self._soft_step(batch, train=False)


class SoftLeakageDataModule(base_train.DataModule):
    def setup(self, stage=None):
        self.train_loader, self.val_loader = get_soft_leakage_dataloaders(
            self.config,
            is_ddp=False,
...
base_train.LightningModule = SoftStage1LightningModule
base_train.DataModule = SoftLeakageDataModule
base_train.main()
