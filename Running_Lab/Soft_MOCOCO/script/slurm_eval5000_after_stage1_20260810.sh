#!/usr/bin/env bash
# Evaluate the final Soft_MOCOCO Stage1 checkpoint after successful training.
...
#SBATCH --job-name=soft_mococo_e5k
#SBATCH --output=/gpfs/home1/wjs6800/SIALab/PNFlowTSE/contrastive_momentum/Running_Lab/Soft_MOCOCO/exp/20260801_soft_mococo/slurm-eval-%j.out

set -euo pipefail

ROOT="/gpfs/home1/wjs6800/SIALab/PNFlowTSE/contrastive_momentum"
LAB_DIR="$ROOT/Running_Lab/Soft_MOCOCO"
PY="${PY:-/home1/wjs6800/miniconda3/envs/pnflowtse/bin/python}"
RUN_DIR="$LAB_DIR/exp/20260801_soft_mococo"
CONFIG="$LAB_DIR/configs/config_tfgridnet_soft_leakage.yaml"
CHECKPOINT="$RUN_DIR/stage1_tfgridnet/checkpoints/tfgridnet_best.ckpt"
OUT="$RUN_DIR/evaluation/results.json"
RUNTIME_CONFIG="$RUN_DIR/config_eval_runtime.yaml"
...
cd "$ROOT"
export PYTHONPATH="$LAB_DIR:$ROOT:$ROOT/Running_Lab/PN_MOCOCO:${PYTHONPATH:-}"
...
"$PY" Running_Lab/Soft_MOCOCO/script/run_eval.py \
  --config "$CONFIG" \
