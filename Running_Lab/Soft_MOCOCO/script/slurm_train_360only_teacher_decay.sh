#!/usr/bin/env bash
# Standard Soft_MOCOCO Stage0: train-clean-360 only, teacher 0.1 -> 0.03.
...
#SBATCH --job-name=soft360_td_s0
#SBATCH --output=exp/20260829_soft_mococo_360only_teacher_decay/stage0_moco/slurm-%j.out

set -euo pipefail

RUN_ROOT="$PWD/exp/20260829_soft_mococo_360only_teacher_decay"
CONFIG="$PWD/configs/config_soft_moco_360only_teacher_decay.yaml"
PYTHON_BIN="/home1/wjs6800/miniconda3/envs/pnflowtse/bin/python"

export MASTER_PORT="${MASTER_PORT:-30160}"
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-4}"
export MKL_NUM_THREADS="${MKL_NUM_THREADS:-4}"
export NUMBA_CACHE_DIR="${NUMBA_CACHE_DIR:-/tmp/soft360_teacher_decay_numba_cache_${USER:-user}}"
...
cp -f "$CONFIG" "$RUN_ROOT/config_soft_moco_360only_teacher_decay.yaml"
...
echo "[Soft_MOCOCO] teacher_weight=0.1->0.03 cosine"
