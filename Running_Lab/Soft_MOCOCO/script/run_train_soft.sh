#!/usr/bin/env bash
# Purpose: run Soft_MOCOCO Stage0 and Stage1 with orthogonal leakage loss.
# Run from: Running_Lab/Soft_MOCOCO
# Single GPU: CUDA_VISIBLE_DEVICES=0 bash script/run_train_soft.sh all
# Multi GPU: CUDA_VISIBLE_DEVICES=0,1,2,3 bash script/run_train_soft.sh all
...
if [[ "$MODE" == "tfgridnet" || "$MODE" == "all" ]]; then
  python script/run_tfgridnet_soft.py --config configs/config_tfgridnet_soft.yaml 2>&1 | tee exp/20260801_soft_mococo/stage1_tfgridnet/train.log
fi
