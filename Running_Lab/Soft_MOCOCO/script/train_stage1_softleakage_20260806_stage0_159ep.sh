...
#
# Purpose:
#   Train and evaluate Soft_MOCOCO Stage1 from the preserved Stage0 epoch-159 encoder.
#
# Run from:
#   contrastive_momentum/Running_Lab/Soft_MOCOCO
#
# Multi GPU:
#   CUDA_VISIBLE_DEVICES=0,1,2,3 MASTER_PORT=29959 \
...
case "$MODE" in
  all|train|eval) ;;
  *)
    echo "Usage: bash script/train_stage1_softleakage_20260806_stage0_159ep.sh [all|train|eval]" >&2
    exit 1
    ;;
esac

PY="${PY:-python}"
RUN_DIR="exp/20260806_stage0_159ep_softleakage_test"
TFGRID_CFG="configs/config_tfgridnet_soft_leakage_20260806_stage0_159ep_test.yaml"
STAGE0_CKPT="$RUN_DIR/stage0_moco/checkpoints/pn_encoder_159ep.pt"
...
require_file "$STAGE0_CKPT" "Freeze the Stage0 epoch-159 export before submitting Stage1."
...
run_train() {
  echo "[Soft_MOCOCO] ===== Stage1: epoch-159 Soft-leakage TFGridNet ====="
