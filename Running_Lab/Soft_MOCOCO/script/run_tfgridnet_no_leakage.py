...
runtime = Path(config["train"]["log_dir"]).resolve().parent / "config_tfgridnet_no_leakage_runtime.yaml"
runtime.parent.mkdir(parents=True, exist_ok=True)
runtime.write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")

# Lightning's DDP child processes re-execute sys.argv[0].  Keep this as the
# PN_MOCOCO absolute path because Soft_MOCOCO does not contain train_tfgridnet.py.
sys.argv = [str(pn_mococo / "train_tfgridnet.py"), "--config", str(runtime)]
