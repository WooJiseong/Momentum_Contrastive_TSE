#!/usr/bin/env bash
# Purpose: train and evaluate Stage1 without the Soft-leakage regularizer.
# Run from: Running_Lab/Soft_MOCOCO
# Usage: CUDA_VISIBLE_DEVICES=0,1,2,3 bash script/train_stage1_wo_leakage_20260803.sh all
...
case "$MODE" in
  ...
  *)
    echo "Usage: bash script/train_stage1_wo_leakage_20260803.sh [all|train|eval|tfgridnet]"
    exit 1
    ;;
esac

PY="${PY:-python}"
RUN_DIR="${RUN_DIR:-exp/20260803_stage0_50ep_soft_wo_leakage_test}"
TFGRID_CFG="${TFGRID_CFG:-configs/config_tfgridnet_soft_wo_leakage_20260803_stage0_50ep_test.yaml}"
...
snapshot_config() {
  cp "$TFGRID_CFG" "$RUN_DIR/source_config_tfgridnet_wo_leakage.yaml"
}
...
run_tfgridnet() {
  require_file "$STAGE0_CKPT" \
    "Copy the pinned epoch-50 Stage0 export to this exact path before submitting Stage1."
  echo "[Soft_MOCOCO] ===== Stage 1: TFGridNet without leakage loss ====="
  "$PY" script/run_tfgridnet_no_leakage.py --config "$TFGRID_CFG" 2>&1 | tee -a "$RUN_DIR/stage1_tfgridnet/train.log"
}
