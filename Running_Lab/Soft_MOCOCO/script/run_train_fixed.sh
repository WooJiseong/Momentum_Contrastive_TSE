#!/usr/bin/env bash
# Purpose: fixed launcher for Soft_MOCOCO after resolving the base Lightning trainer explicitly.
# Run from: Running_Lab/Soft_MOCOCO
# Single GPU: CUDA_VISIBLE_DEVICES=0 bash script/run_train_fixed.sh moco
# Multi GPU: CUDA_VISIBLE_DEVICES=0,1,2,3 bash script/run_train_fixed.sh all
