...
base_spec = importlib.util.spec_from_file_location(
    "pn_mococo_base_train_tfgridnet",
...
class PNIndivStage1LightningModule(base_train.LightningModule):
    """Stage1 separator training with target-orthogonal individual leakage loss."""
    ...
    def _nuisance_sources(
        ...
        if "nuisance_sources" not in batch:
            raise KeyError(
                "Stage1 leakage loss requires actual sample[1:] nuisance_sources, not "
                "negative enrollment or aggregate background."
            )
        ...
        if self.verify_nuisance_contract and not self._nuisance_contract_checked:
            ...
            if not torch.isfinite(nuisance_sources).all() or max_error > allowed_error:
                raise RuntimeError(
                    "Invalid nuisance contract: sum(nuisance_sources) must reconstruct "
    ...
    def _pn_indiv_step(self, batch: dict, train: bool):
        ...
        if self.fail_on_nonfinite and not torch.isfinite(loss).all():
            raise FloatingPointError(
                "PN_Indiv_MOCOCO Stage1 produced a non-finite loss: "
                f"si_sdr={float(si_sdr_loss.detach()):.6g}, "
    ...
    def training_step(self, batch, batch_idx):
        return self._pn_indiv_step(batch, train=True)

    def validation_step(self, batch, batch_idx):
        return self._pn_indiv_step(batch, train=False)


class PNIndivStage1DataModule(base_train.DataModule):
...
base_train.LightningModule = PNIndivStage1LightningModule
base_train.DataModule = PNIndivStage1DataModule
