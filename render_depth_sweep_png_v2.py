...
def read(path, tag):
    ...
    acc.Reload()
    return [(x.step, x.value) for x in acc.Scalars(tag)] if tag in acc.Tags().get("scalars", []) else []
...
def render(tag, output):
    ...
    for i in range(6):
        ...
        draw.text((8, y - 7), f"{hi - (hi - lo) * i / 5:.3f}", fill="#444444")
    colors = {"baseline": "#1f77b4", "mococo": "#ff7f0e"}
    for name, points in series.items():
        if len(points) < 2:
            continue
        condition, depth = name.split("_n")
        coords = []
...
def main():
    out = ROOT / "exp/20260801_tensorboard_depth_sweep/png"
    for tag in TAGS:
        render(tag, out / f"{tag}.png")
